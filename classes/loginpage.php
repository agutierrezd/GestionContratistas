...
class LoginPage extends RunnerPage
{
	...
	protected function getSelectSQL( $skipPasswordCheck, $strUsername, $strPassword, $loginSet, $cipherer )
	{
		...
		return "select * from ".$this->connection->addTableWrappers("contractor_master")
	...
	public function LoginWithSP( $plugin, $token, $addNewUser = true )
	{
		...
		$sqlHead = "select * from ".$this->connection->addTableWrappers( "contractor_master" ) ." where ";
		$sql = array();
		if( $userIdField ) {
			$sql []= $sqlHead
		...
		else if( $addNewUser )
		{
			...
			$blobfields = array();
			$retval = DoInsertRecord( "contractor_master", $data, $blobfields, $this );
			if( !$retval )
				return false;
