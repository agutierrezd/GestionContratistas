...
{	
	...
	function fillGridData() 
	{
		...
		while( $data )
		{
			...
			$username = $data["contractor_doc_id"];
			$groups_sate = array();
			...
			$row["username"] = runner_htmlspecialchars($username);
				$displayUserName = $data["contractor_nombresfull"];
			...
			$this->users[$userid]["displayUserName"] = $displayUserName;
				$emailUser = $data["contractor_email"];
	...
	function fillMembers()
	{
		...
		$sql = "select ". $grConnection->addFieldWrappers( "UserName" )
			...
			." from ". $grConnection->addTableWrappers( "gestioncontratistas_ver_103_ugmembers" ) 
	...
	function fillGroups()
	{
		...
		$sql = "select ". $grConnection->addFieldWrappers( "GroupID" ) .", "
			...
			." from ". $grConnection->addTableWrappers( "gestioncontratistas_ver_103_uggroups" ) 
	...
	function updateUserGroups($user, $groups)
	{
		...
		$grConnection = $cman->getForUserGroups();
		// It's expected that $this->tName is equal to 'admin_members' so the page's db connection is used #9875		
		$membersWTableName = $grConnection->addTableWrappers( "gestioncontratistas_ver_103_ugmembers" );
