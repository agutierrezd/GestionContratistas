...
function checkTableName($shortTName, $type=false)
{
	...
	if ("tipo_cta_banco" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tipo_regimen" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	...
	if ("tparam_tipo_ct_arl" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tparam_genero" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
...
function ReadUserPermissions($userID = "")
{
	...
	if($userID != "Guest")
	{
		...
		$sql = "select ".$gConn->addFieldWrappers( "GroupID" )
			...
			." from ". $gConn->addTableWrappers( "gestioncontratistas_ver_103_ugmembers" )
	...
	$sql = "select ". $gConn->addFieldWrappers( "TableName" )
		...
		." from ". $gConn->addTableWrappers( "gestioncontratistas_ver_103_ugrights" )
